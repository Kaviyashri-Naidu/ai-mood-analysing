...
export default function Index() {
  ...
  const [history, setHistory] = useState<Array<{text: string, mood: string, score: number, date: string}>>([]);
  ...
  const getMoodGradient = (s: number) => {
    if (s > 3) return ["#fbbf24", "#f59e0b"];
    if (s > 0) return ["#34d399", "#10b981"];
    if (s === 0) return ["#94a3b8", "#64748b"];
    if (s > -3) return ["#fb923c", "#f97316"];
  ...
  const analyzeMood = () => {
    ...
    words.forEach(word => {
      ...
      if (customWords[cleanWord]) {
        boostScore += customWords[cleanWord];
      }
    });
